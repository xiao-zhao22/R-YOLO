    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(0,0,0), rotated=False):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                #     lineType=cv2.LINE_AA,
                # )
            ##标记函数：去除标识框，将文字改为‘Y’增加线宽，代替线框用来标注每个钢筋目标
     
            ccc1 = p1[0]  + p2[0]
            ccc2 = p1[1]  + p2[1]
            cv2.putText(self.im, 'Y',  (int(ccc1//2-155), int(ccc2//2+68)), 0, self.lw / 6, (255, 0, 0), thickness=self.lw+3, lineType=cv2.LINE_AA)
            """ xxx = ccc1/5120
            yyy = ccc2/2880
            www = (p2[0]-p1[0])/2560
            hhh = (p2[1]-p1[1])/1440  """
    ##print("xxx的类型是：%s,www的类型是：%s" % (type(xxx),type(www)))
    ##print("0 %f %f %f %f" % (xxx,yyy,www,hhh))
